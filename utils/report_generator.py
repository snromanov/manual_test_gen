...
class CoverageReportGenerator:
    """Генерирует HTML-отчёт с матрицей трассировки требования -> тест-кейсы."""
...
<div class="container">
    ...
    # ISTQB technique display names and icons
    TECHNIQUE_NAMES = {
    ...
    def generate(self, result: dict, output_path: Path) -> str:
        ...
        for i, req_id in enumerate(sorted(matrix.keys())):
            info = matrix[req_id]
            row_class = "" if info['testcase_count'] > 0 else 'class="uncovered"'

            if info['testcases']:
                tc_chips = "".join(f'<span class="tc-chip">{tc}</span>' for tc in info['testcases'])
                tc_list = f'<div class="tc-list">{tc_chips}</div>'
            else:
                tc_list = '<span class="tc-chip" style="background:rgba(255,56,96,0.1);color:var(--neon-red);border-color:rgba(255,56,96,0.2)">NONE</span>'
            ...
            matrix_rows += f"""
                <tr {row_class} style="animation-delay:{0.05*(i+1):.2f}s">
                    ...
                    <td>{info['title']}</td>
                    <td>{tc_list}</td>
                    <td style="text-align:center">{chk(info['has_positive'])}</td>
                    <td style="text-align:center">{chk(info['has_negative'])}</td>
                    <td style="text-align:center">{chk(info['has_boundary'])}</td>
                </tr>"""

        # ISTQB technique bars
    ...
    <div class="card rec-card" style="animation-delay:0.45s">
        ...
        # Gauge offset: 502 = full circle circumference (2*pi*80)
        ...
        html = self.HTML_TEMPLATE.format(
            ...
            status_text="ALL REQUIREMENTS COVERED" if is_pass else "COVERAGE INSUFFICIENT",
